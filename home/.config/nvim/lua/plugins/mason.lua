vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
        ...
        vim.keymap.set('n', 'gf',
                       '<cmd>vsplit | lua vim.lsp.buf.definition()<cr>', opts)
        ...
        if (client) then
            ...
            if client:supports_method('textDocument/documentHighlight') then
                ...
                local augroup = vim.api.nvim_create_augroup('lsp_highlight',
                                                            {clear = false})

                vim.api
                    .nvim_clear_autocmds({buffer = opts.buf, group = augroup})
...
vim.lsp.config('eslint', {
    ...
    root_markers = {'.git'},
    cmd = {
        'vscode-eslint-language-server', '--max-old-space-size=8192', '--stdio'
    },
    settings = {
        debug = true,
        rootMarkers = {'.git/'},
...
for _, pkg_identifier in ipairs({
    'prettierd', 'luaformatter', 'tree-sitter-cli', 'pgformatter'
}) do
