...
def parse_plugins_from_vimrc
  plugins = []
  File.new('.vim/vim-plug.vim').each do |line|
    line.scan(/Plug\s+["'](.+?)["']/).each do |plug|
      plugins << convert_to_link_hash(plug.first)
    end
  end
...
def plugins
  ...
  File.new('.vim/vim-plug.vim').each do |line|
    if line =~ /^\s*Plug\s+["'](.+)["']/
      plugin_info = fetch_plugin_info($1, "./vim/plugin/#{1}/.vim")
      plugins << plugin_info
    end
    print '.'
  end
...
def repo_info(user, name)
  ...
  begin
    response = URI.open(api_url, "Authorization" => "token #{ENV['GITHUB_TOKEN']}").read
